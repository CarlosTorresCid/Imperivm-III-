# Secuencia 3 — `GuardPosts_Main`

`GuardPosts_Main` es la Sequence encargada de gestionar todos los `GGuardPost` del mapa como posiciones defensivas persistentes.
...
│
└── defensa añadida por GuardPosts_Main
...
El sistema principal se ejecuta cada medio segundo.

Esto permite:

- detectar enemigos rápidamente;
- bloquear órdenes manuales;
- controlar el estado de captura;
- detener inmediatamente la regeneración cuando comienza un ataque.
...
Cada 500 ms se recorren todos los Guard Posts.
...
# 23. Limpieza de referencias del propietario anterior

Antes de crear la nueva escolta se ejecuta:
...
La intención es eliminar referencias muertas del grupo anterior.

La Sequence parte de la premisa de que, en una conquista normal, los 10 guardianes del propietario anterior ya han muerto porque `AllowCapture(true)` sólo se habilita cuando `garrison.count == 0`.
...
No existe regeneración progresiva en el momento de conquistar.

El nuevo propietario empieza directamente con:
...
Si una unidad se ha alejado demasiado:

```cpp
post.DistTo(defender) > post.range
```
...
Así los guardianes salen automáticamente a combatir.
...
# 38. Por qué estas tropas no pueden utilizarse como ejército

Las órdenes se vuelven a imponer cada:

```text
500 ms
```

Si el jugador intenta enviar un guardián lejos:

```text
orden manual del jugador
↓
máximo ~500 ms
↓
GuardPosts_Main vuelve a imponer la orden defensiva
```
...
evita que una IA propietaria se lleve los guardianes como parte de una ofensiva normal.

Estas unidades forman parte funcional del puesto.
...
En su lugar los guardianes regresan a dos puntos externos.
...
# 54. Qué ocurre si el Guard Post cambia de propietario de forma inesperada mientras aún quedan guardianes

El diseño presupone que esto no debería ocurrir porque:
...
Por tanto una captura normal sólo debería ser posible cuando:

```text
garrison.count == 0
```

Sin embargo, si alguna otra Sequence o comportamiento externo ejecutase manualmente:

```cpp
post.SetPlayer(...)
```

mientras aún quedan guardianes, el bloque:

```cpp
owner != lastOwner
```

detectaría el cambio y generaría una nueva guarnición.

El código no elimina expresamente guardianes vivos del propietario anterior antes de crear los nuevos.

### Consecuencia

En condiciones normales no es un problema.

Pero si existen otras Sequences capaces de forzar cambios de propietario sobre `GGuardPost`, podría producirse coexistencia temporal de guardianes antiguos y nuevos dentro del mismo Group.

Conviene evitar cambios manuales de propietario fuera de este sistema.
...
5. `GuardPosts_Main` debe ejecutarse una sola vez con `Autorun allowed`.
...
`GuardPosts_Main` implementa este ciclo:
