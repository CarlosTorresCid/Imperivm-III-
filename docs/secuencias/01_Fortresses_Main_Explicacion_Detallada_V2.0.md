...
|---|---:|---|
| `REGEN_INTERVAL` | 20.000 ms | Una baja puede regenerarse cada 20 segundos de paz acumulada |
| `DEFENDER_LEVEL` | 20 | Nivel de toda unidad creada por el sistema |
| `CONTROL_INTERVAL` | 2.000 ms | Frecuencia del controlador principal |

El núcleo del sistema revisa cada fortaleza aproximadamente **una vez cada 2 segundos**.

La versión v2.0 reduce así la carga respecto a revisiones anteriores. Las órdenes defensivas no se reescriben sin necesidad: el código comprueba el comando actual antes de volver a emitir `move`, `advance` o `enter_tent`.
...
## 12. Inicialización después de la primera conquista

La Sequence conserva el contador:
...
Por tanto, cuando el bucle detecta por primera vez que un Outpost normal ya pertenece a un Player 1..8, el incremento es de 2.000 ms y supera inmediatamente el umbral de 1.000 ms.

En la práctica, **no existe ya una espera adicional de un segundo repartida en dos ciclos**. La guarnición se crea en el mismo ciclo en que `Fortresses_Main` detecta la primera conquista, aunque esa detección puede producirse hasta unos 2 segundos después del cambio de propietario por la frecuencia general del controlador.

`FG_FirstDelay` se conserva en el código por compatibilidad con la arquitectura anterior y se reinicia a `0` al completar la inicialización.
...
Mientras `FG_CarthageCleanup` siga por encima de cero, el Settlement se revisa en cada ciclo del controlador (aproximadamente cada 2 segundos).

Si aparece un `CVillager`, se elimina.

El contador parte de 5.000 ms y se descuenta en bloques de 2.000 ms. Por ello la ventana efectiva queda cuantizada por el ciclo de control y puede abarcar aproximadamente hasta 6 segundos. Al agotarse:
...
El controlador revisa la guarnición aproximadamente cada 2 segundos.

Durante un ataque, una unidad que se aleje más allá de `fort.range` recibe una orden `move` de regreso. Dentro del radio, el sistema sólo vuelve a imponer `advance` cuando la unidad no está ya ejecutando `advance`, `attack` o `engage`.

Por tanto, la guarnición continúa estando ligada funcionalmente al Outpost, pero la versión v2.0 evita reescribir órdenes compatibles en cada ciclo.
...
impide que la IA estratégica incorpore estas tropas a sus ejércitos normales.

Estas unidades están concebidas como una extensión funcional de la fortaleza, no como un ejército gratuito para campañas ofensivas.
...
El controlador lo comprueba cada 2 segundos, pero sólo emite `enter_tent` si el defensor todavía no está dentro y no tiene ya esa misma orden.

---

## 27.1. Excepción: el EOutpost neutral no regenera

La guarnición neutral personalizada de Egipto —10 `EHorusWarrior` + 10 `EAnubisWarrior`— es **finita mientras el fortín siga neutral**.
...
La regeneración de esa fortaleza comienza únicamente después de que pase a un propietario real y entre en el sistema post-conquista.
