...
30. [Caso de estudio: arquitectura técnica del modo Zombies](#30-caso-de-estudio-arquitectura-técnica-del-modo-zombies)
...
## 12.3 `IntArray` existe en scripts del motor, no está confirmado en Sequence

El script recuperado declara:

```cpp
IntArray posX, posY, races;
```

y utiliza indexación:
...
Esto corrige cualquier afirmación absoluta de que “CKS/VS no tiene arrays”. **El motor sí los tiene.**

Lo que sigue sin estar confirmado es que `IntArray` y ese mismo modelo de arrays sean utilizables desde
una Sequence de escenario. En los `.BFHP` inspeccionados no se encontró precedente equivalente.
...
Sequence .vs
→ disponibilidad de IntArray no confirmada
```
...
## 14.3 `EnvReadInt` no admite claves dinámicas

La siguiente forma no compila con la firma disponible en Sequence:

```cpp
EnvReadInt(obj, "KEY"+index)
```

La solución utilizada es desenrollar las lecturas con claves literales.

La escritura equivalente con `EnvWriteInt` sí se ha utilizado con claves dinámicas, por lo que la limitación observada es asimétrica.
...
```cpp
active = EnvReadInt(state, "HW_ACTIVE"+H);
```
...
| `Contains(obj)` | pertenencia; aparece en una Sequence real del mapa de trabajo (`HordeTacticalAI_Main_BREACH`), aunque no se había encontrado en las campañas oficiales iniciales |
...
## 16.7 Tipos y constructores: `point` vs `Point`

Se ha confirmado la distinción:
...
`point` es el tipo observado; `Point(x,y)` es el constructor.

Además, scripts del motor usan:
...
pero `IntArray` **no está confirmado desde Sequence**.
...
Ejemplo ya consolidado:

```text
EnvReadInt(obj, "KEY"+index)
```

no es una forma válida con la firma disponible en Sequence, mientras que la escritura dinámica sí se ha utilizado.
...
3. Si `AddToGroup("NombreNoDeclarado")` crea de forma segura un Group nuevo o exige que el nombre exista antes en `map.obj.xml`.
4. Semántica completa de `UnitFlags`.
5. Códec exacto de los bloques comprimidos de `HMMSYS PackFile` y del formato `LZIS`.
6. Disponibilidad de `IntArray` y otras colecciones del motor dentro del compilador de Sequences del editor.
7. Efectos secundarios completos de `Townhall.SetPlayer()` sobre recursos, población, tropas y sentinelas.
8. Cuerpo y reglas completas de `townhall_sentries_control.vs`.
9. Significado exacto del tercer parámetro de `ObjList.Siege(target,n,param)`.
10. Algoritmo interno de `BestGate`; el helper `"siege"` lo usa por nosotros, pero el criterio exacto sigue sin recuperarse.
11. Significado completo de `GAIKA`, aunque ya se han recuperado `GetGAIKAStrat.vs`, `GS_Siege.vs` y sus firmas en `AI.INI`.
12. Fórmula de recuperación nativa de loyalty sugerida por `LoyaltyIncreasePerUnit`.
13. Acceso directo desde una Sequence escrita a mano a `Gate.IsBroken`/`IsVeryBroken`: están confirmadas dentro de AI Helpers, pero no había precedente directo en los 48 BFHP oficiales iniciales.
14. Condición de victoria cuando existe un Player técnico 9-14 con unidades vivas pero sin Townhall propio.
15. Rendimiento/estabilidad real con cientos de unidades técnicas y muchas hordas simultáneas durante partidas de varias horas.
16. Eficacia práctica del mecanismo anti-atasco por “wiggle” del prototipo de hordas; el código existe, pero necesita validación larga en partida.
17. Consistencia de nombres de Groups dinámicos con padding (`HW_H1` frente a `HW_H01`) en el prototipo actual.
...
## 25.7 Arrays: corrección y límite de contexto

La investigación de hordas no encontró arrays en Sequences, pero otros scripts nativos sí recuperaron:
...
```text
arrays en el lenguaje del motor
→ CONFIRMADOS

arrays en una Sequence BFHP
→ NO CONFIRMADOS
```

Para una Sequence de producción, el patrón conservador sigue siendo usar:

```text
ObjList
variables Unit/Building
Groups predefinidos
bloques explícitos por instancia
```

hasta validar `IntArray` en el editor.

## 25.8 Slots explícitos para muchas hordas

El precedente real del motor para muchas instancias independientes es duplicar estado:
...
Por analogía, una Sequence puede reservar slots:

```cpp
ObjList hordeUnits_1;
Unit hordeLeader_1;
Building hordeTarget_1;
bool hordeActive_1;

ObjList hordeUnits_2;
Unit hordeLeader_2;
Building hordeTarget_2;
bool hordeActive_2;
```

Es verboso, pero evita depender de arrays no confirmados en Sequence o de Groups con nombres creados
dinámicamente.

## 25.9 Groups dinámicos: qué está confirmado y qué no
...
en una Sequence real.

No está confirmado que:

```cpp
u.AddToGroup("NombreNuncaDeclarado");
```

cree automáticamente un Group nuevo si ese nombre no existe en `map.obj.xml`.

Por eso los nombres de Group generados dinámicamente no deben ser la única fuente de identidad de un
ejército hasta superar ese test.
...
# 30. Caso de estudio: arquitectura técnica del modo Zombies

El mapa de trabajo contiene ya un prototipo de hordas que sirve como caso real de arquitectura avanzada
con múltiples Sequences cooperando. Este capítulo documenta **la infraestructura**, no fija su balance
como regla universal del juego.

## 30.1 Tres Sequences ya desplegadas

En el BFHP de trabajo se encontraron:

```text
ForumCaptureReward_Main
HordeWaves_Main_BREACH
HordeTacticalAI_Main_BREACH
```
...
```text
ForumCaptureReward_Main
→ recompensa por primera captura de Foro neutral

HordeWaves_Main_BREACH
→ spawn, ronda, objetivo, movimiento y estado principal de hordas

HordeTacticalAI_Main_BREACH
→ Gates, arietes y recuperación anti-atasco
```

La existencia del código está confirmada; una partida completa de varias horas sigue siendo necesaria
para validar estabilidad y balance a escala máxima.

## 30.2 Ocho puntos de aparición reales

Groups encontrados:

| Group | Marcador | Coordenadas |
|---|---|---:|
| `HordeSpawn_01` | `Stonehenge1` | `(27711,22090)` |
| `HordeSpawn_02` | `Shrine` | `(8248,31326)` |
| `HordeSpawn_03` | `Stonehenge2` | `(2889,18772)` |
| `HordeSpawn_04` | `Stonehenge1` | `(5465,10041)` |
| `HordeSpawn_05` | `Stonehenge1` | `(14776,741)` |
| `HordeSpawn_06` | `Stonehenge1` | `(7012,1082)` |
| `HordeSpawn_07` | `Stonehenge1` | `(13330,16691)` |
| `HordeSpawn_08` | `Shrine` | `(27567,31120)` |

Los objetos decorativos se utilizan únicamente como anclas de posición.

## 30.3 Player técnico 12

Escaneo del BFHP antes del spawn:

```text
player 9-14 → 0 objetos estáticos
player 15   → miles de objetos/refs neutrales
player 16   → cientos
```
...
es una elección limpia para ese mapa: cualquier Query sobre player 12 identifica únicamente objetos del
sistema de hordas.

Sigue abierta una cuestión distinta: **si unidades vivas de player 12 sin Townhall pueden interferir con
la condición de victoria de una partida normal**. Debe comprobarse experimentalmente.

## 30.4 24 slots explícitos y persistencia local

El prototipo no depende de un array de estructuras. Duplica estado:
...
La Sequence declara esas variables una vez y las conserva durante su `while(1)`.

Es el mismo patrón de ingeniería ya observado en:

```text
ROutpost1 ... ROutpost9
```

cuando el lenguaje o el compilador de Sequence no ofrecen una colección estructurada suficientemente
probada.

## 30.5 Comunicación entre Sequences mediante `EnvWriteInt`/`EnvReadInt`

Hallazgo arquitectónico importante:

```text
Sequence A
→ escribe estado en un objeto compartido

Sequence B
→ lee ese estado
```

Ejemplo conceptual del prototipo:

```cpp
EnvWriteInt(boardBuilding, "HW_T01", targetIndex);
```

otra Sequence:

```cpp
targetIndex = EnvReadInt(boardBuilding, "HW_T01");
```

El `Building` actúa como una “pizarra” persistente accesible por ambas ejecuciones.

Esto demuestra que dos Sequences independientes pueden coordinarse sin acceder a las variables locales
de la otra.

## 30.6 Groups como canal de identidad entre Sequences

La Sequence de waves añade unidades a Groups y la Sequence táctica recupera:

```cpp
Group("HW_H01").GetObjList();
```

Existe una discrepancia pendiente de verificación entre nombres con y sin padding (`HW_H1` frente a
`HW_H01`) en transcripciones del prototipo. Es un test crítico: si el nombre escrito y el leído no
coinciden, la IA táctica no encontrará la horda.

## 30.7 Objetivos: 8 capitales fijas frente a 18 ciudades reales

El prototipo inicial limita candidatos a:

```text
CapitalForum_P1 ... CapitalForum_P8
```

Eso no cubre una ciudad inicialmente neutral que haya sido conquistada.

El mecanismo técnico ya confirmado para un universo dinámico es:
...
{
    towns.AddList(
        ClassPlayerObjs("BaseTownhall", p).GetObjList()
    );
}
...
Así el sistema ve cualquier `BaseTownhall` que **actualmente** pertenezca a un jugador 1-8,
independientemente de su dueño inicial.

## 30.8 Distancia no equivale a accesibilidad

La selección por:

```cpp
forum.DistTo(referenceUnit)
```

es geométrica. No existe una API confirmada de:
...
Por eso una ciudad “más cercana” puede estar separada por otra ciudad-puerta o por terreno. El sistema
de nodos del mapa debe tratarse como una capa de diseño adicional si se necesita respetar accesibilidad
territorial real.

## 30.9 IA táctica desplegada: dos Gates por horda

`HordeTacticalAI_Main_BREACH` reserva dos slots de Gate por horda.

Búsqueda:

```text
cada 5 ticks
→ muestrear 1 de cada 8 unidades
→ Buildings en radio 1200
→ IsHeirOf("Gate")
→ Gate enemiga
```
...
```cpp
R1 = Place("BCatapultUnit", u.pos, 12).AsUnit();
R1.SetNoAIFlag(true);
R1.SetCommand("attack", G1);
```

Mientras el ariete vive, se reemite el ataque periódicamente. Tras considerar la Gate destruida, el
prototipo usa un cooldown de 25 s y después devuelve a toda la horda a:

```cpp
SetCommand("advance", target.pos);
```

**Corrección posterior importante:** ese prototipo histórico vigilaba `IsAlive()` para algunas Gates.
La investigación de `GS_Siege.vs` y `SIEGE GATE.VS` mostró que el motor usa `IsBroken` como condición de
brecha. Para una revisión futura, el helper nativo o `IsBroken` deben preferirse frente a esperar muerte
total del objeto.

## 30.10 Anti-atasco genérico por “wiggle”

Cada 10 ticks, dos unidades centinela por horda sirven como sensores.
...
y la unidad prácticamente no se ha desplazado, el prototipo:

```text
1er indicio → reintenta advance
atasco persistente → rota una fase 1..4
→ calcula un punto cardinal a ~700
→ mueve allí unidades aliadas próximas en radio ~600
→ vuelve a intentar la ruta normal
```

La optimización evita ejecutar una costosa prueba sobre todas las unidades de todas las hordas.

La **existencia del mecanismo está confirmada**; su eficacia real frente a todos los obstáculos sigue
siendo un resultado experimental pendiente.

## 30.11 Recompensa por Foro frente a recompensa por Horda

`ForumCaptureReward_Main` implementa:

```text
Foro inicialmente neutral
→ primera captura por jugador 1-8
→ dos ejércitos de 50
→ una sola recompensa por Foro durante la partida
```

Eso es distinto de una mecánica:

```text
horda concreta eliminada
→ premiar al defensor que la detuvo
```

La primera está desplegada; la segunda requiere vincular cada slot de horda con su objetivo/owner y
entregar la recompensa al morir el último miembro. El manual las mantiene separadas para no presentar
una mecánica de diseño como si ya estuviera confirmada.

## 30.12 Escalado y riesgos

El prototipo contiene configuraciones de hasta cientos de unidades por horda y muchas instancias
simultáneas. El hecho de que el código exista no demuestra que el motor mantenga FPS/pathfinding estable
durante horas.

Riesgos abiertos prioritarios:

```text
Player 12 y victoria
rendimiento con 100-250 unidades × muchas hordas
nombres de Group con padding
ciudades neutrales obligatorias de paso
anti-atasco en terreno real
```

## 30.13 Tests de mayor valor

1. dejar vivo player 12 sin Townhall y ganar una partida normal;
2. probar `RunAIHelper("siege",...)` y `"siege gate"` sobre una copia de prueba;
3. sustituir `IsAlive` por el criterio `IsBroken`/helper;
4. atacar una ciudad de doble Gate;
5. confirmar `HW_H01`/`HW_H1` en el Source real del editor;
6. ejecutar 3+ horas con varias hordas y medir estabilidad;
7. comprobar una ciudad neutral obligatoria situada de camino a otra ciudad objetivo;
8. validar la recompensa por horda cuando dos hordas atacan la misma ciudad.
...
- el prototipo real de modo Zombies desplegado en el mapa de trabajo y su coordinación multi-Sequence;
...
IntArray confirmado en scripts de motor, no todavía en Sequence
...
prototipo Zombies: 3 Sequences coordinadas, 24 slots, Player 12 y anti-atasco documentados
```
...
4. **El lenguaje del motor sí tiene arrays.**
   `IntArray` está confirmado en scripts nativos, aunque no en Sequences.
